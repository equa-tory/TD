    ...
    public class ScriptManager
    {
        ...
        public string PrintTicket(string id, string title, string createdIso, string timestampIso, string position)
        {
            try
            {
                ...
                ticket.Type          = "";
                ticket.Number        = 0;
                ticket.DisplayNumber = title;  // use the display name directly (title here is already the ticket number)
                ticket.Created       = created;
                ticket.Timestamp     = timestamp;
                ticket.Position      = pos;
                ticket.HasSlot       = !string.IsNullOrEmpty(timestampIso); // empty for non-time types
